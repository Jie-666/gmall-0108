...
//            System.out.println(response);
    ...
    private SearchResponseVo parseResult(SearchResponse response) {
        ...
        //获取聚合结果集中的品牌id聚合，解析出品牌列表
        ParsedLongTerms brandIdAgg = (ParsedLongTerms) aggregations.get("brandIdAgg");
        List<? extends Terms.Bucket> brandBuckets = brandIdAgg.getBuckets(); //获取品牌id聚合结果中的桶
        if (!CollectionUtils.isEmpty(brandBuckets)) {
            List<BrandEntity> brandEntities = brandBuckets.stream().map(bucket -> { //把每个桶转化为品牌
                BrandEntity brandEntity = new BrandEntity();
                brandEntity.setId(bucket.getKeyAsNumber().longValue()); //桶中的key就是品牌id
                //获取品牌id聚合中的子聚合
        ...
        if (!CollectionUtils.isEmpty(categoryBuckets)) {
            List<CategoryEntity> categoryEntities = categoryBuckets.stream().map(bucket -> {
                ...
                categoryEntity.setId(bucket.getKeyAsNumber().longValue()); //桶中的key就是分类的id
                //获取分类id聚合中的子聚合
        ...
        //把attrId的桶转化为SearchResponseAttrValueVo集合
        if (!CollectionUtils.isEmpty(attrIdAggBuckets)) {
            List<SearchResponseAttrValueVo> searchResponseAttrValueVos = attrIdAggBuckets.stream().map(bucket -> {
                ...
                if (!CollectionUtils.isEmpty(attrValueAggBuckets)) {
                    //有多个值   数据类型为：private List<String> attrValues;
    ...
    private SearchSourceBuilder builder(SearchParamVo paramVo) {
        ...
        if (!CollectionUtils.isEmpty(props)) {
            props.forEach(prop -> {
                ...
                //对规格参数过滤的条件字符串进行截取，获取attrId,attrValue
                String[] attr = StringUtils.split(prop, ":");
                if (attr != null && attr.length == 2) { //分割后的数组长度必须是2，才处理
                    //每一个规格参数过滤条件中的元素队形一个嵌套查新
